...
def train_single_model_worker(
    ...
    """
    Train a SINGLE model in its own container.

    - Uses the same training logic as model_training_agent used to.
    - Saves model + metrics to the shared volume.
    - Returns metrics + selection score to the orchestrator.
    """
    import os
    import json
    import joblib
    ...
    try:
        ...
        if train_func is None:
            return {
                "status": "error",
                "message": f"Unsupported model for task {task}: {model_name}",
            }

        # ---- Train model using shared helpers ----
        result = train_func(
            X_train, y_train,
            X_test, y_test,
            params,
        ...
        # ---- Save model + metrics to shared volume ----
        ...
        # ---- Compute generalization gap + selection score ----
        if task == "regression":
            ...
            gap_penalty = max(0.0, generalization_gap)
            # higher accuracy → lower score is better (negative accuracy + penalty)
            score_for_selection = -test_metric + 0.3 * gap_penalty
        ...
        logger.exception(f"train_single_model_worker failed for {model_cfg}: {e}")
        return {
            "status": "error",
...
def model_training_agent(
    ...
    # NEW: fan-out to per-model workers in parallel containers
    worker_handles = []
    for model_cfg in model_list:
        h = train_single_model_worker.spawn(
            user_id=user_id,
            version=version,
            target_column=target_column,
            task=task,
    ...
    for h in worker_handles:
        ...
        score_for_selection = res.get("score_for_selection")
        if score_for_selection is None:
            continue

        if best_score is None or score_for_selection < best_score:
            best_score = score_for_selection
            best_model_name = res.get("model_name")
            best_model_path = res.get("model_path")
            best_metrics_path = res.get("metrics_path")
            best_generalization_gap = res.get("generalization_gap")
    # --- END replaced block ---
...
5. Mix of target-focused and feature–feature charts:
    ...
    try:
        ...
        summary_path = os.path.join(models_folder, f"models_summary_v{version}.json")
        with open(summary_path, "w") as f:
            json.dump(models_summary, f, indent=2)

    except Exception as e:
        logger.warning(f"Failed to write models_summary_v{version}.json: {e!r}")
    ...
    for col in df.columns:
        ...
        if semantic_type == "numeric_continuous":
            ...
            if len(col_series) > 0:
                ...
                # ---- Detect standardized data (z-scores) ----
                if abs(mean) < 0.1 and abs(std - 1) < 0.1:
                    # z-scores usually fall in [-3, 3] but allow some tolerance
                    min_allowed = -5
                    max_allowed = 5

                # ---- Detect [0, 1] scaled data ----
                elif 0 <= actual_min <= 0.05 and 0.95 <= actual_max <= 1.05:
                    min_allowed = 0
                    max_allowed = 1

                # ---- Detect [-1, 1] scaled data ----
                elif -1.05 <= actual_min <= -0.95 and 0.95 <= actual_max <= 1.05:
                    min_allowed = -1
                    max_allowed = 1

                else:
                    # ---- Raw numeric logic (your original code) ----
                    min_allowed = actual_min * 0.5
                    max_allowed = actual_max * 1.5

                    # round to nearest 10s
                    max_allowed = math.ceil(max_allowed / 10) * 10
                    min_allowed = math.floor(min_allowed / 10) * 10
...
def api_models_summary(user_id: str, version: int):
    ...
    # ----------------------------------------------------------------------
    # 1. FAST PATH: If AutoML saved a compact summary → use it
    # ----------------------------------------------------------------------
    summary_path = os.path.join(models_folder, f"models_summary_v{version}.json")
    if os.path.exists(summary_path):
        ...
        all_models = summary.get("all_models", [])
    ...
    # ----------------------------------------------------------------------
    # 2. FALLBACK: Infer task, load metrics_v files, recompute best model
    # ----------------------------------------------------------------------
    pre_folder = get_user_preprocessed(user_id)
    cleaned_path = os.path.join(pre_folder, f"cleaned_v{version}.parquet")
    metadata_path = os.path.join(pre_folder, f"metadata_v{version}.json")
    ...
    # Collect all metrics files for this version
    metrics_files = [
        f
    ...
    # Same scoring logic as model_training_agent
    def _score_model(m: dict, task: str) -> float | None:
        ...
        if task == "regression":
            if "rmse" not in train_m or "rmse" not in test_m:
                return None
            gap = test_m["rmse"] - train_m["rmse"]
            return test_m["rmse"] + 0.3 * max(0.0, gap)

        else:  # classification
            if "accuracy" not in train_m or "accuracy" not in test_m:
                return None
            gap = train_m["accuracy"] - test_m["accuracy"]
            return -test_m["accuracy"] + 0.3 * max(0.0, gap)

    # Pick best model
    best_model = None
    best_score = None
    for m in all_models:
        s = _score_model(m, task_type)
        if s is None:
            continue
